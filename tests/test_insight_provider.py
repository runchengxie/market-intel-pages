...
class ProviderTests(unittest.TestCase):
    def test_http_diagnostics_keep_only_allowlisted_codes(self):
        body = {"error": {"status": "INVALID_ARGUMENT", "message": "API key not valid: secret-value",
                         "details": [{"reason": "API_KEY_INVALID", "metadata": {"key": "secret-value"}}]}}
        error = HTTPError("https://example.test/?key=secret-value", 400, "secret-value", {},
                          io.BytesIO(json.dumps(body).encode()))
        with patch("scripts.insight_provider.urlopen", side_effect=error), self.assertRaises(HTTPError) as caught:
            generate({}, "prompt", "gemini", "test", "secret-value")
        self.assertEqual({"http_status": 400, "api_status": "INVALID_ARGUMENT", "reason": "API_KEY_INVALID"},
    ...
    def test_gemini_requests_schema_and_ignores_thought_parts(self):
        def respond(request, **kwargs):
            ...
            return Response({"candidates": [{"finishReason": "STOP", "content": {"parts": [
                {"text": "private reasoning", "thought": True}, {"text": '{"ok": true}'}]}}]})
    ...
    def test_retryable_failure_recovers_but_auth_failure_is_not_retried(self):
        response = Response({"candidates": [{"finishReason": "STOP", "content": {"parts": [{"text": "{}"}]}}]})
        calls = []
        def temporary(request, **kwargs):
            ...
            return response
        with patch("scripts.insight_provider.urlopen", side_effect=temporary), patch("scripts.insight_provider.time.sleep"):
    ...
    def test_truncated_generation_is_rejected(self):
        response = Response({"candidates": [{"finishReason": "MAX_TOKENS", "content": {"parts": [{"text": "{}"}]}}]})
