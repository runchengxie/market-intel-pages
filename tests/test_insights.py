...
def sources():
    return [
        {"id": "e1", "date": "2026-09-14", "kind": "evening", "title": "复盘",
         "sections": [{"title": "盘面", "paragraphs": ["生成时间: 2026-09-14 19:00",
             "上涨率 56.3%；成交额/历史中位 0.82x。", "[WARN] 新闻条目缺失。"]}]},
        {"id": "m1", "date": "2026-09-14", "kind": "morning", "title": "晨报",
         "sections": [{"title": "隔夜", "paragraphs": ["生成时间: 2026-09-15 07:00",
             "SMH -4.75%，QQQ -0.80%。"]}]},
    ]


def analysis():
    return {"overview": {"text": "个股修复还需要成交确认。", "evidence_ids": ["e1:s0:p1"]},
            "changes": [{"text": "上涨率 56.3%。", "evidence_ids": ["e1:s0:p1"]}],
            "tensions": [{"text": "半导体走弱。", "evidence_ids": ["m1:s0:p1"]}],
            "watchpoints": [{"question": "成交能否改善？", "metric": "volume_ratio", "operator": ">",
                             "threshold": 0.82, "evidence_ids": ["e1:s0:p1"]}]}
...
class InsightContractTests(unittest.TestCase):
    ...
    def test_verification_uses_first_later_evening_and_preserves_original(self):
        rows = sources()
        note = {"id": "note1", "date": "2026-09-14", "as_of": "2026-09-15T07:00:00+08:00",
                "analysis": analysis()}
...
class GenerationTests(unittest.TestCase):
    def test_replacement_keeps_verifying_the_original_archived_condition(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            run(rp, ip, api_key="test", archive_dir=p / "archive", force=True, generator=lambda *args: replacement)
            later = copy.deepcopy(rows[0])
            later.update(id="e2", date="2026-09-15")
            later["sections"][0]["paragraphs"] = ["生成时间: 2026-09-15 19:00", "成交额/历史中位 0.90x"]
            rp.write_text(json.dumps({"schema_version": "market_intel_pages.reports.v1", "reports": rows + [later]}))
            run(rp, ip, api_key=None, archive_dir=p / "archive")
            outcomes = [json.loads(f.read_text(encoding="utf-8")) for f in (p / "archive/outcomes").glob("*.json")]
    ...
    def test_generation_cache_revision_and_append_only_archive(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            rows = sources()
            rp.write_text(json.dumps({"schema_version": "market_intel_pages.reports.v1", "reports": rows}))
            args = dict(provider="gemini", model="test", api_key="test", archive_dir=p / "archive",
