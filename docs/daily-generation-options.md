...
核对日期：2026-09-24。本文说明报告从上游生产到公开网站的交接方式、数据检查和每日生成方案。初次盘点与后续整改分开记录。
...
美股日报是独立于 A 股晨晚报的补充材料。平台优先使用美国财政部每日收益率表计算当日 bp 变动，缺失时才回退到 FRED；CPI、PCE、失业率和非农就业变动仍来自 FRED，逐项标注观测日。经逐条核实、与私有草稿哈希绑定的报道可补入指数收盘涨跌和研究解释，未通过审核的候选不进入报告。部署侧校验 JSON 后生成 `publication: public` 清单，独立美股 Pages 发布器才允许导入。网页展示最近一次日报与核实来源，并对次日修订作明确标记；没有审核材料的日子仍显示指数和研究缺项。导入时由同一份公开白名单数据生成 Markdown 和可保存的纯文本版；网页另以带零线的暖色横条图展示已核实的指数涨跌和当日美债收益率变动，不把滞后观测值画成当日图表。
