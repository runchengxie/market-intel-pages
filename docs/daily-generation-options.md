...
美股日报是独立于 A 股晨晚报的补充材料。平台优先使用美国财政部每日收益率表计算当日 bp 变动，缺失时才回退到 FRED；CPI、PCE、失业率和非农就业变动仍来自 FRED，逐项标注观测日。经逐条核实、与私有草稿哈希绑定的报道可补入指数收盘涨跌和研究解释，未通过审核的候选不进入报告。部署侧校验 JSON 后生成 `publication: public` 清单，独立美股 Pages 发布器才允许导入。网页展示最近一次日报与核实来源，并对次日修订作明确标记；没有审核材料的日子仍显示指数和研究缺项。
