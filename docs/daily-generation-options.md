...
美股宏观日报是独立于 A 股晨晚报的补充材料。平台从 FRED 获取国债收益率变动、CPI、PCE、失业率和非农就业变动，并逐项附观测日；部署侧校验后才替换当前 JSON，Pages 发布器将其导入 `data/market_daily_report.json`。网站单独展示最近一次日报，不受 A 股日期筛选影响。指数行情和研究解释仍属缺项，收益率尚未公布当日数据时标记延迟，不能把该模块视为完整美股市场日报。相关改动在跨仓 PR 合并并部署前，生产网站不会出现新增内容。
