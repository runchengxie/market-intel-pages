...
六图交接使用 `market_intel.a_share_charts.v1`。上游 `chart-candidate` 只产生私有候选，不构成事实审核或公开授权；仅在逐点核对原始网页、日期、数值及使用许可后，独立签发带内容哈希且标明 `publication: public` 的清单。Pages 侧预览后导入，构建时只复制五个公开报告日期内的已审核 JSON，其他候选和旧 PNG 不会进入站点。目前没有可公开的六图清单，因此正式网页应显示缺项。静态表保持无脚本可读，交互图按用户展开和当前报告 ID 加载。
